...
const availableComponents = [
  { id: "keyLeadMetrics", name: "Key Lead Metrics", component: KeyLeadMetrics, width: 1, height: 1 },
  { id: "leadsBySource", name: "Leads by Source", component: LeadsBySource, width: 2, height: 1 },
  { id: "overallLeadFunnel", name: "Overall Lead Funnel", component: OverallLeadFunnel, width: 2, height: 1 },
  { id: "topNewLeads", name: "Top New Leads", component: TopNewLeads, width: 1, height: 1 },
  { id: "teamTaskSummary", name: "Team Task Summary", component: TeamTaskSummary, width: 1, height: 1 },
];
...
const userRole=localStorage.getItem('userrole')
    ...
    return (
      <div
        ...
        className={`bg-white rounded-xl shadow-lg transition-all duration-300 ${
          isFullscreen ? "fixed inset-0 z-50 m-[50px] p-6" : "p-6"
  ...
  {/* Remove Component Button - Hides in Fullscreen Mode */}
  {!isFullscreen && (
    <button
  ...
  // Group components into rows (max 2 per row, considering width)
  const getRows = () => {
    const rows = [];
    let currentRow = [];
    let currentWidth = 0;
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
              ...
              <div className="max-h-64 overflow-y-auto">
                {availableComponents
                  .filter((comp) => !addedComponents.includes(comp.id))
                  .map((comp) => (
                    <button
                      key={comp.id}
                      onClick={() => handleAddComponent(comp.id)}
                      className="w-full text-left p-3 mb-2 bg-gray-50 hover:bg-indigo-50 rounded-lg transition-colors border border-gray-200"
                    >
                      {comp.name}
                    </button>
                  ))}
