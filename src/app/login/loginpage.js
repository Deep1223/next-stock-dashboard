...
export default function LoginPage() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                } else if (result.role === "sales_person") {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-white py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-[400px] w-full bg-white rounded-2xl shadow-2xl p-8 space-y-8 transform transition duration-300 ease-in-out border border-gray-100">
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
                    <div className="space-y-6">
                        <div>
                            ...
                            <input id="email" type="email" {...register("email", { required: "Email is required", pattern: { value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i, message: "Invalid email address" } })} className={`mt-1 w-full px-4 py-3 border ${errors.email ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-indigo-500"} rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:border-transparent transition duration-200 placeholder-gray-400 text-gray-900`} placeholder="Enter your email" />
